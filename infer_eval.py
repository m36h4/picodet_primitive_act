...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The file path containing path of image to be infered. "
             "Valid only when --infer_dir is given."
    )
    ...
    parser.add_argument(
        "--slice_infer",
        action='store_true',
        help="Whether to slice the image and merge the inference "
             "results for small object detection."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='nms',
        help="Combine method of the sliced images' detection results, "
             "choose in ['nms', 'nmm', 'concat']."
    )
...
def get_test_images(infer_dir, infer_img, infer_list=None):
    ...
    IMPORTANT:
    This version preserves the order of --infer_list.
    The original PaddleDetection implementation uses a set(),
    which destroys the ordering.
    """
    ...
    # IMPORTANT:
    # Use a list instead of a set so inference order is preserved.
    images = []
    ...
    if infer_list:
        ...
        for ext in exts:
            images.extend(
                glob.glob(
                    '{}/*.{}'.format(infer_dir, ext)
                )
            )

        # Deterministic order when infer_list is not provided.
        images = sorted(images)

    assert len(images) > 0, \
        "no image found in {}".format(infer_dir)

    logger.info(
        "Found {} inference images in total.".format(len(images))
    )
