"""
Compute Recall @ FPPI (False Positives Per Image) at a fixed IoU threshold,
from COCO-format ground truth + predictions (e.g. PaddleDetection's bbox.json).

This is a standard metric for single/few-class detectors (esp. small-object
detectors like ball detection) where you care about "how many true objects
do I catch, if I only tolerate N false positives per image on average" --
distinct from COCO's own AP metric, which averages over the full
precision-recall curve and doesn't let you pick an explicit operating point.

Definitions used here:
  - IoU threshold: a prediction counts as a match to a GT box only if
    IoU(pred, gt) >= iou_thresh (default 0.4, as requested).
  - Matching: greedy, highest-confidence-first, one prediction can match
    at most one GT box and vice versa (standard COCO-style matching).
  - FPPI curve: sweep the confidence threshold from high to low. At each
    threshold, compute total false positives / total images = FPPI, and
    recall = matched GT / total GT.
  - Recall @ FPPI=X: recall at the confidence threshold where FPPI is
    closest to (and does not exceed, by default) X.
  - FP / FN counts: reported as raw totals (not rates) at the same
    operating point selected for Recall @ FPPI, so you can see exactly
    how many false positives and missed ground-truth boxes that implies.

Usage:
  python3 recall_at_fppi.py \
      --gt instances_eval.json \
      --pred bbox.json \
      --iou 0.4 \
      --fppi 0.05

Requires: pycocotools (pip install pycocotools)
"""
import argparse
import json
from collections import defaultdict

import numpy as np
from pycocotools.coco import COCO


def compute_iou(box_a, box_b):
    """boxes in [x, y, w, h] format (COCO convention)."""
    ax1, ay1, aw, ah = box_a
    bx1, by1, bw, bh = box_b
    ax2, ay2 = ax1 + aw, ay1 + ah
    bx2, by2 = bx1 + bw, by1 + bh

    inter_x1 = max(ax1, bx1)
    inter_y1 = max(ay1, by1)
    inter_x2 = min(ax2, bx2)
    inter_y2 = min(ay2, by2)

    inter_w = max(0.0, inter_x2 - inter_x1)
    inter_h = max(0.0, inter_y2 - inter_y1)
    inter_area = inter_w * inter_h

    area_a = aw * ah
    area_b = bw * bh
    union = area_a + area_b - inter_area
    if union <= 0:
        return 0.0
    return inter_area / union


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--gt", required=True, help="Path to ground-truth COCO json")
    ap.add_argument("--pred", required=True, help="Path to predictions (COCO-format bbox.json)")
    ap.add_argument("--iou", type=float, default=0.4, help="IoU threshold for a match")
    ap.add_argument("--fppi", type=float, default=0.05,
                     help="Target false-positives-per-image operating point")
    ap.add_argument("--category-id", type=int, default=None,
                     help="Restrict to one category id (default: all categories combined, "
                          "correct for single-class 'ball' setups)")
    args = ap.parse_args()

    coco_gt = COCO(args.gt)
    with open(args.pred) as f:
        predictions = json.load(f)

    # --- Join GT and predictions by IMAGE FILENAME, not numeric image_id ---
    gt_img_id_to_name = {img["id"]: img["file_name"] for img in coco_gt.dataset["images"]}
    gt_name_to_img_id = {name: img_id for img_id, name in gt_img_id_to_name.items()}

    def pred_image_name(p, id_to_name_from_pred_run):
        if "file_name" in p:
            return p["file_name"]
        return id_to_name_from_pred_run.get(p["image_id"])

    pred_id_to_name = gt_img_id_to_name

    if args.category_id is not None:
        predictions = [p for p in predictions if p["category_id"] == args.category_id]
        gt_img_ids = coco_gt.getImgIds()
        gt_ann_ids = coco_gt.getAnnIds(imgIds=gt_img_ids, catIds=[args.category_id])
    else:
        gt_img_ids = coco_gt.getImgIds()
        gt_ann_ids = coco_gt.getAnnIds(imgIds=gt_img_ids)

    n_images = len(gt_img_ids)
    gt_anns = coco_gt.loadAnns(gt_ann_ids)

    # Group GT boxes by FILENAME
    gt_by_name = defaultdict(list)
    for ann in gt_anns:
        if ann.get("iscrowd", 0) == 1:
            continue
        name = gt_img_id_to_name[ann["image_id"]]
        gt_by_name[name].append(ann["bbox"])
    total_gt = sum(len(v) for v in gt_by_name.values())

    # Resolve every prediction's image_id to a filename, using the map above
    resolved_predictions = []
    n_unresolved = 0
    for p in predictions:
        name = pred_image_name(p, pred_id_to_name)
        if name is None:
            n_unresolved += 1
            continue
        resolved_predictions.append({**p, "_image_name": name})

    matched_names = set(gt_by_name.keys()) & set(p["_image_name"] for p in resolved_predictions)

    print(f"Images (GT): {n_images}")
    print(f"Ground-truth boxes: {total_gt}")
    print(f"Predictions (all confidences): {len(predictions)}")
    print(f"Predictions with unresolved image_id (dropped): {n_unresolved}")
    print(f"Distinct GT filenames: {len(gt_by_name)}")
    print(f"Distinct prediction filenames (resolved): {len(set(p['_image_name'] for p in resolved_predictions))}")
    print(f"Filenames present in BOTH GT and predictions: {len(matched_names)}")
    if len(matched_names) == 0:
        raise SystemExit(
            "No filenames matched between GT and predictions at all -- the join key "
            "itself is broken (e.g. predictions use a different image_id numbering "
            "than this GT file, or file_name strings differ in path/case). "
            "Print a few raw examples from both files and compare by hand before retrying."
        )

    predictions = resolved_predictions

    if total_gt == 0:
        raise SystemExit("No ground-truth boxes found -- check --gt path / --category-id.")
    if len(predictions) == 0:
        raise SystemExit("No predictions found -- check --pred path / --category-id.")

    # Sort all predictions by confidence, descending
    predictions = sorted(predictions, key=lambda p: p["score"], reverse=True)

    # For each image (by name), track which GT boxes have already been matched.
    matched_gt = {name: [False] * len(boxes) for name, boxes in gt_by_name.items()}

    n_tp = 0
    n_fp = 0
    # curve now carries the raw cumulative counts alongside the rates, so we
    # can report absolute FP/FN totals at whichever point we pick later --
    # not just recall and fppi ratios.
    curve = []  # (score, recall, fppi, cumulative_tp, cumulative_fp)

    for pred in predictions:
        name = pred["_image_name"]
        pred_box = pred["bbox"]
        score = pred["score"]

        gt_boxes = gt_by_name.get(name, [])
        best_iou = 0.0
        best_idx = -1
        for i, gt_box in enumerate(gt_boxes):
            if matched_gt[name][i]:
                continue
            iou = compute_iou(pred_box, gt_box)
            if iou > best_iou:
                best_iou = iou
                best_idx = i

        if best_iou >= args.iou and best_idx >= 0:
            matched_gt[name][best_idx] = True
            n_tp += 1
        else:
            n_fp += 1

        recall = n_tp / total_gt
        fppi = n_fp / n_images
        curve.append((score, recall, fppi, n_tp, n_fp))

    # Find the highest-recall point on the curve with fppi <= target.
    # fppi is monotonically non-decreasing as the threshold is lowered,
    # so the last point satisfying the constraint is the best one.
    best_point = None
    for score, recall, fppi, tp, fp in curve:
        if fppi <= args.fppi:
            best_point = (score, recall, fppi, tp, fp)
        else:
            break

    print(f"\n=== Recall @ FPPI={args.fppi}, IoU={args.iou} ===")
    if best_point is None:
        score0, recall0, fppi0, tp0, fp0 = curve[0]
        fn0 = total_gt - tp0
        print(
            f"No confidence threshold achieves FPPI <= {args.fppi} "
            f"(even the single highest-confidence prediction already exceeds it). "
            f"Lowest achieved FPPI: {fppi0:.4f} at score={score0:.4f}, "
            f"recall at that point: {recall0:.4f}\n"
            f"At that point -- TP: {tp0}, FP: {fp0}, FN: {fn0} (out of {total_gt} GT boxes)"
        )
    else:
        score, recall, fppi, tp, fp = best_point
        fn = total_gt - tp
        print(f"Confidence threshold: {score:.4f}")
        print(f"Recall: {recall:.4f}")
        print(f"Achieved FPPI: {fppi:.4f} (target was {args.fppi})")
        print(f"Total FP: {fp}")
        print(f"Total FN: {fn}  (out of {total_gt} GT boxes)")
        print(f"Total TP: {tp}")

    print("\n=== Recall / FP / FN at other reference FPPI points (for context) ===")
    for target in [0.01, 0.05, 0.1, 0.5, 1.0]:
        point = None
        for score, recall, fppi, tp, fp in curve:
            if fppi <= target:
                point = (score, recall, fppi, tp, fp)
            else:
                break
        if point:
            score, recall, fppi, tp, fp = point
            fn = total_gt - tp
            print(f"  FPPI<={target:<5} -> recall={recall:.4f}  FP={fp:<5} FN={fn:<5}  "
                  f"(score>={score:.4f}, actual fppi={fppi:.4f})")
        else:
            print(f"  FPPI<={target:<5} -> not achievable (min fppi = {curve[0][2]:.4f})")


if __name__ == "__main__":
    main()
